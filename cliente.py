...
#     #convierte todo a string
#     tipo = separacion[0].decode("utf-8")
# #---------------------------------------------------------------------           
# #---------------------------------------------------------------------           
# #-----aqui se trabajan los audios que son de mi topic-----------------            
# #---------------------------------------------------------------------            
# #---------------------------------------------------------------------             
# #     if tipo == "audio":
# #         grupo = separacion[1].decode("utf-8")
# #         usuario_destino = separacion[2].decode("utf-8")
# #         tamamo_audio = separacion[3].decode("utf-8")#se convierte a entero
# #         usuario_emisor  = separacion[4].decode("utf-8")
# #         mensaje = separacion[5]
# #         longitud = len(separacion)
    
# #         logging.info("Enviado por usuario : " + str(usuario_emisor))
# #         logging.info("tamaño del archivo: " + str(tamamo_audio))
# #         now = datetime.now()
# #         timestamp = datetime.timestamp(now)#la hora en formato UNIX (epoch)
# #         with open(str(timestamp)+".wav", "wb") as f:
# #             f.write(mensaje)
# #             f.close()
# #         os.system('aplay '+str(timestamp)+".wav",) 
    

# # #---------------------------------------------------------------------           
# # #---------------------------------------------------------------------           
# # #-----aqui se trabajan los textos que son de mi topic-----------------            
# # #---------------------------------------------------------------------            
# # #---------------------------------------------------------------------  
# #     elif tipo == "usuarios":
# #         grupo = separacion[1].decode("utf-8")
# #         usuario_destino = separacion[2].decode("utf-8")
# #         usuario_emisor  = separacion[3].decode("utf-8")
# #         mensaje = separacion[4].decode("utf-8")
        
# #         logging.info("Enviado por usuario : " + str(usuario_emisor))
# #         logging.info("mensaje: " + str(mensaje))    
...
# client = mqtt.Client() #Nueva instancia de cliente
# client.on_connect = on_connect #Se configura la funcion "Handler" cuando suceda la conexion
# client.on_message = on_message #Se configura la funcion "Handler" que se activa al llegar un mensaje a un topic subscrito
# client.username_pw_set(MQTT_USER, MQTT_PASS) #Credenciales requeridas por el broker
# client.connect(host=MQTT_HOST, port = MQTT_PORT) #Conectar al servidor remoto

# def publishData(topicRoot, topicGrupo, topcUsuario, value, qos = 0):
#     topic = topicRoot + "/" + topicGrupo + "/" + topcUsuario
...
# client.subscribe(("usuarios/02/"+SENDER, qos))

# client.subscribe(("audio/02/"+SENDER, qos))

# client.subscribe(("audio/02/"+MI_SALA+"/#", qos))

# client.subscribe(("salas/02/"+MI_SALA+"/#", qos))
...
#             bit_grupo = bytes(Nogrupo, "ascii")
#             bit_usuario = bytes(NoID, "ascii")
#             bit_tama = bytes(texto, "ascii")            
#             separador = bytes('$', "ascii")
#             identi = bytes(topic, "ascii")
#             emisor = bytes(SENDER, "ascii")
#             #se abre el archivo de audio y se une en un raw para ser enviado por completo
 
#             Trama11 = identi+separador+bit_grupo+separador+bit_usuario+separador+emisor+separador+bit_tama
#             publishData(topic,Nogrupo, NoID,Trama11 )                        
...
#             print('\n\nEnviando el siguiente texto:  {!s}'.format(bit_tama))
#             sock.sendall(bit_tama) #Se envia utilizando "socket.sendall" 
...
#                 print("[+] Sending file...")
#                 datos = f.read()    
#                 #Trama11 = identi + separador+ bit_grupo +separador + bit_usuario +separador+bit_tama+separador+emisor+separador+datos
#                 Trama11 = FTR + separador + bit_usuario + separador + bit_tama
#             publishData(topic,Nogrupo, NoID,Trama11 )
...
#             entrada= round(time.time(),2)
#             contador = contador + 1
#             client.publish("comandos/02",final,qos)
...
# logging.basicConfig(
#     #se cambia DEBUG por info, para que muestre de info para arriba
#     level = logging.DEBUG, 
#     format = '[%(levelname)s] (%(threadName)-10s) %(message)s'
#     )
...
# # hilo 1 para contar los segundo
# t1 = threading.Thread(name = 'Contador de 1 segundo',
#                         target = contador_seg,
#                         args = (range(0,11),0.5, str(201112345)),
#                         daemon = True
#                         )
...
#                 datos = f.read()    
#                 #Trama11 = identi + separador+ bit_grupo +separador + bit_usuario +separador+bit_tama+separador+emisor+separador+datos
#                 Trama11 = FTR+separador+bit_usuario+separador
#             publishData(topic,Nogrupo, NoID,Trama11 )
